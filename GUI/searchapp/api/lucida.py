...
class LucidaAPI(BaseStreamingAPI):
    def __init__(self):
        super().__init__()
        self.site_name = "lucida"
        self.base_url = None
    ...
    def _get_album_scraper(self, media_item: Entries):
        """Build and fetch a lucida AlbumScraper from an album Entries item."""
        scr_mod = importlib.import_module(f"VibraVid.{get_folder_name()}.lucida.scrapper")
        dl_mod = importlib.import_module(f"VibraVid.{get_folder_name()}.lucida.downloader")
        AlbumScraper = getattr(scr_mod, "AlbumScraper")
        build_client = getattr(dl_mod, "_build_client")

        raw_data = media_item.raw_data if isinstance(media_item.raw_data, dict) else {}
        url = str(media_item.url or raw_data.get("url") or "").strip()
        if not url:
            return None

        scraper = AlbumScraper(url, client=build_client())
        scraper.fetch()
        return scraper

    def search(self, query: str) -> List[Entries]:
        """Resolve a service/lucida url into Entries for the GUI."""
    ...
    def get_series_metadata(self, media_item: Entries) -> Optional[List[Season]]:
        """For albums: resolve the tracklist and return it as a single Season."""
        ...
        try:
            ...
            if scraper is None:
                logger.warning("Lucida album metadata skipped: url not found for '%s'", media_item.name)
                return None
            ...
            logger.exception("Lucida get_series_metadata failed for '%s'", media_item.name)
            return None

    def start_download(self, media_item: Entries, season: Optional[str] = None, episodes: Optional[str] = None) -> bool:
        """Delegate the download to the lucida service."""
        search_fn = self._get_search_fn()
